...
const ArtistProfile = (props: PropsType) => {
  const { artistInfo, setDeleteArtistModeOn, setEditArtistModeOn } = props;
    ...
    <div className={cx("artistPage")}>
      <div className={cx("profile_top")}>
        <NavLink to="/artists" className={cx("comeBackButton")}>
          <Button value="back" theme={theme} type="arrow" width="100px" />
        </NavLink>
        {isLoggedIn && (
          <div className={cx("deleteEditBlock")}>
            <button type="button" onClick={() => setEditArtistModeOn(true)}>
              <img className={cx("deleteEditIcon")} src={edit} alt="edit" />
            </button>
            <button type="button" onClick={() => setDeleteArtistModeOn(true)}>
              <img
        ...
        <div className={cx("mainPhoto")}>
          ...
          >
            ...
            <div className={cx("dash", `dash_${theme}`)}>
              <img
                src={theme === themes.light ? dashLight : dashDark}
                alt="dash"
                width="30px"
              />
            </div>
            <div
              className={cx("artistDescription", {
                // функция learn more закомментирована пока artistInfo.description на backend-e < 256 символов
...
export const NoArtworks: FC<NoArtworksPropsType> = ({
  addArtwork,
  setAddEditPictureModeOn,
}) => {
  const { theme, toggleTheme } = useContext(ThemeContext);
  return (
    <div className={cx("noArtworks", `noArtworks_${theme}`)}>
      <div className={cx("noArtworksContainer")}>
        <img src={noImagePlug} alt="noImagePlug" width="168px" height="151px" />

        <button
          type="button"
          className={cx(
            "addFirstPictureButton",
        ...
        >
          <img src={addDark} alt="addPicture" width="16px" height="16px" />
        </button>
      </div>
    </div>
  );
...
export const NoImagePlug: FC<{ theme: string }> = ({ theme }) => {
  return (
    <div className={cx("NoImagePlug", `NoImagePlug${theme}`)}>
      <div className={cx("NoImagePlugContainer")}>
        <img src={noImagePlug} alt="noImagePlug" width="168px" height="151px" />
      </div>
    </div>
  );
